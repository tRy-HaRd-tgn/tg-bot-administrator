...
class ScheduleModal {
    ...
    validatePeriod() {
        ...
        if (startDate.value && endDate.value) {
            ...
            if (start > end) {
                this.showError(endDate, endError, 'Дата окончания не может быть раньше даты начала');
                isValid = false;
            }
        }
    ...
    renderCalendar() {
        ...
        if (!startDateStr) {
            calendarContainer.innerHTML = '<div class="text-center text-muted">Сначала выберите дату начала</div>';
            return;
        }
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            const onClickAttr = clickable ? `onclick="scheduleModal.toggleDate('${dateStr}')"` : '';
            
            calendarHTML += `
                <div class="${classes.join(' ')}" ${onClickAttr} data-date="${dateStr}">
                    ${day}
                </div>
            `;
        }
    ...
    selectAllDates() {
        ...
        const startDate = new Date(startDateStr);
        const endDate = endDateStr ? new Date(endDateStr) : new Date(startDate.getTime() + 30 * 24 * 60 * 60 * 1000); // 30 дней по умолчанию
        const today = new Date();
        today.setHours(0, 0, 0, 0);
        ...
        for (let date = new Date(Math.max(startDate.getTime(), today.getTime())); date <= endDate; date.setDate(date.getDate() + 1)) {
            const dateStr = date.toISOString().split('T')[0];
            this.selectedDates.add(dateStr);
            if (this.timeMode === 'multiple') {
                this.individualTimes[dateStr] = '12:00';
    ...
    selectDaysByType(dayFilter) {
        ...
        const startDate = new Date(startDateStr);
        const endDate = endDateStr ? new Date(endDateStr) : new Date(startDate.getTime() + 30 * 24 * 60 * 60 * 1000);
        const today = new Date();
        today.setHours(0, 0, 0, 0);
    ...
    handleTimeModeChange() {
        const selectedMode = document.querySelector('input[name="timeMode"]:checked');
    ...
    updateIndividualTimes() {
        ...
        if (this.selectedDates.size === 0) {
            container.innerHTML = '<div class="no-dates-selected"><i class="fa-solid fa-info-circle me-2"></i>Сначала выберите даты публикаций</div>';
            return;
        }
    ...
    updatePreview() {
        ...
        if (this.selectedDates.size === 0) {
            previewContainer.innerHTML = '<div class="no-schedule-preview"><i class="fa-solid fa-calendar-xmark me-2"></i>Выберите даты и время для отображения превью</div>';
            return;
        }
        ...
        const previewItems = sortedDates.map(dateStr => {
            const date = new Date(dateStr);
            const formattedDate = date.toLocaleDateString('ru-RU', {
                weekday: 'long',
                day: 'numeric',
            ...
            const time = this.timeMode === 'single' ? globalTime : (this.individualTimes[dateStr] || '12:00');
    ...
    validateSettings() {
        ...
        if (this.selectedDates.size === 0) {
            this.showNotification('error', 'Ошибка', 'Необходимо выбрать хотя бы одну дату для публикации');
            return false;
        }
        ...
        if (this.timeMode === 'multiple') {
            for (const dateStr of this.selectedDates) {
                if (!this.individualTimes[dateStr]) {
                    this.showNotification('error', 'Ошибка', 'Не указано время для всех выбранных дат');
                    return false;
                }
            }
        }
    ...
    saveSettings() {
        ...
        try {
            ...
            this.showNotification('success', 'Успех', 'Расписание публикаций сохранено');
            
        } catch (error) {
            console.error('Ошибка при сохранении настроек расписания:', error);
            this.showNotification('error', 'Ошибка', 'Не удалось сохранить настройки расписания');
        }
    }
    ...
    loadSettings(settings) {
        ...
        try {
            ...
            const timeModeRadio = document.querySelector(`input[name="timeMode"][value="${this.timeMode}"]`);
            if (timeModeRadio) {
