...
class NewCampaignManager {
  ...
  bindEvents() {
    ...
    // Повторение
    this.bindElement("repeatEnabled", "change", (e) =>
    ...
    document.querySelectorAll(".repeat-tab").forEach((tab) => {
      tab.addEventListener("click", (e) =>
        this.switchRepeatTab(e.target.dataset.tab)
      );
    });

    // Поиск чатов
    this.bindElement("chatsSearch", "input", (e) =>
      this.searchChats(e.target.value)
    );

    // Медиафайлы
    this.bindElement("mediaUploadArea", "click", () => this.triggerFileInput());
    this.bindElement("mediaUploadArea", "dragover", (e) =>
            ...
            <div class="chat-item ${
              isSelected ? "selected" : ""
                ...
                <div class="chat-item-content">
                    ...
                    <div class="chat-info">
                        <h6 class="chat-title">${Utils.escapeHtml(
                          chat.title
                        )}</h6>
                        <div class="chat-badges">
                            <span class="chat-badge ${typeConfig.badgeClass}">${
      typeConfig.label
    }</span>
                            ${
                              chat.bot_is_admin
                ...
                <div class="chat-actions">
                    <button type="button" class="chat-select-btn ${
                      isSelected ? "selected" : ""
                    }" 
                            onclick="newCampaignManager.toggleChatSelection('${
                              chat.id
                            }')">
                        <i class="fa-solid ${
                ...
                ${
                  chat.is_forum
                    ? `
                    <div class="forum-topics" style="display: ${
  ...
  updateChatUI(chatId) {
    ...
    selectBtn.innerHTML = `<i class="fa-solid ${
      isSelected ? "fa-check" : "fa-plus"
    }"></i>`;
          ...
          .map(
            (chat) => `
                        <div class="selected-chat-item">
                            <span class="chat-name">${Utils.escapeHtml(
                              chat.title
                            )}</span>
                            ${
                              chat.thread_id
  ...
  validateFile(file) {
    ...
    if (file.size > maxSize) {
      Utils.showNotification(
        "error",
        "Ошибка",
        `Файл ${file.name} слишком большой (макс. 20MB)`
      );
      return false;
    }
  ...
  renderMediaPreview() {
    ...
    preview.innerHTML = `
            ...
            <div class="media-preview-grid">
                ${this.selectedFiles
                  .map((file, index) => this.renderMediaItem(file, index))
                  .join("")}
            </div>
        `;
  }
    ...
    return `
            <div class="media-item" data-index="${index}">
                ...
                <div class="media-info">
                    ...
                    <div class="media-size">${Utils.formatFileSize(
                      file.size
                    )}</div>
                </div>
  ...
  addButton() {
    ...
    setTimeout(() => {
      const lastButtonItem = document.querySelector(
        `.button-item[data-button-id="${button.id}"] input`
      );
      if (lastButtonItem) {
  ...
  renderButtons() {
    ...
    container.innerHTML = this.buttons
      .map(
        (button) => `
            <div class="button-item" data-button-id="${button.id}">
                <div class="button-inputs">
                    <div class="form-group">
                        ...
                        <input type="text" class="form-input" placeholder="Текст кнопки" 
                               ...
                               oninput="newCampaignManager.updateButton(${
                                 button.id
                               }, 'text', this.value)">
                    </div>
                    <div class="form-group">
                ...
                <button type="button" class="remove-button-btn" onclick="newCampaignManager.removeButton(${
                  button.id
                })">
                ...
                <div class="telegram-message-body">
                    ${
                      this.selectedFiles.length > 0
                        ? this.generateMediaPreviewHtml()
                        : ""
                    }
                    <div class="telegram-message-text">${this.formatMessageText(
                      messageText
                    )}</div>
                    ${this.generateButtonsPreviewHtml()}
                    <div class="telegram-message-time">${new Date().toLocaleTimeString(
                      "ru-RU",
  ...
  generateMediaPreviewHtml() {
    ...
    const photoUrls = this.selectedFiles.map((file) =>
    ...
    return `<div class="telegram-message-media"><div class="telegram-message-media-group">${photoUrls
      .slice(0, 4)
      .map((url) => `<img src="${url}">`)
      .join("")}</div></div>`;
  }
  ...
  generateButtonsPreviewHtml() {
    ...
    const validButtons = this.buttons.filter(
              ...
              .map(
                ...
                <div class="telegram-inline-button-row">
                    <a href="${Utils.escapeHtml(
                      button.url
                    )}" class="telegram-inline-button" target="_blank">
    ...
    return (
      text
        ...
        .replace(
          ...
          '<span style="background: #666; color: #666; border-radius: 3px; padding: 2px 4px; cursor: pointer;" title="Нажмите чтобы показать">$1</span>'
        )
        .replace(
          /<code>(.*?)<\/code>/g,
          '<code style="background: var(--apple-fill-secondary); padding: 2px 4px; border-radius: 3px; font-family: monospace; font-size: 14px;">$1</code>'
        )
        .replace(
          /<pre>(.*?)<\/pre>/g,
          '<pre style="background: var(--apple-fill-secondary); padding: 8px; border-radius: 6px; font-family: monospace; white-space: pre-wrap; border-left: 4px solid var(--apple-blue);">$1</pre>'
        )
        .replace(
          /<a href="([^"]*)">(.*?)<\/a>/g,
          '<a href="$1" style="color: var(--apple-blue); text-decoration: none;" target="_blank">$2</a>'
        )
    );
  }
  ...
  validateDates() {
    ...
    if (startDate && endDate) {
      ...
      if (startDateObj < todayUTC) {
        this.showFieldError(
          "startDate",
          "Дата начала не может быть в прошлом (UTC)"
        );
        return false;
      }
  ...
  async handleSubmit(e) {
    ...
    if (!this.validateForm()) {
      // Проверяем, не связано ли с датами
      const startDateInput = document.getElementById("startDate");
      const endDateInput = document.getElementById("endDate");
      if (!startDateInput || !endDateInput) {
        Utils.showNotification(
          "error",
          "Ошибка",
          "Укажите дату начала и окончания публикации"
        );
      } else {
        Utils.showNotification(
          "error",
  ...
  async submitCampaign(status) {
    try {
      ...
      console.log(
        "Отправляемые кнопки:",
        this.buttons.filter((b) => b.text && b.url)
      );
  ...
  buildFormData(status) {
    ...
    formData.append(
      "disable_preview",
      document.getElementById("disable_preview").checked
    );
    formData.append(
      "disable_notification",
      document.getElementById("disable_notification").checked
    );
    formData.append(
      "protect_content",
      document.getElementById("protect_content").checked
    );
    formData.append(
      "pin_message",
      document.getElementById("pin_message").checked
    );
  ...
  cancel() {
    if (
      confirm(
        "Вы уверены, что хотите отменить создание кампании? Все несохраненные данные будут потеряны."
      )
    ) {
      window.location.href = "/campaigns";
    }
