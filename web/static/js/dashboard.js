...
class Dashboard {
  ...
  updateDashboard(campaignData, chatData) {
    console.log(campaignData, chatData);
    // Обновляем статистику кампаний (удаляем ненужные)
    this.updateElement(
      this.elements.activeСampaigns,
      campaignData.active_campaigns
    );
    this.updateElement(
      this.elements.totalСampaigns,
      campaignData.total_campaigns
    );
...
function fetchChatsStats() {
  fetch("/api/statistics/chats")
    ...
    .then((data) => {
      ...
      document.getElementById("supergroups-count").innerText =
        data.supergroups || 0;
    })
    .catch((error) => {
      console.error("Ошибка при получении статистики чатов:", error);
