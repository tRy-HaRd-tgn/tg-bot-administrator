...
class RepeatModal {
  ...
  bindEvents() {
    ...
    document
      .querySelectorAll('input[name="repeat_interval"]')
      .forEach((radio) => {
    ...
    if (this.modal) {
      this.modal.addEventListener("hidden.bs.modal", () => {
        // Очистка только после полного закрытия модалки
  ...
  handleIntervalChange() {
    const selectedInterval = document.querySelector(
      'input[name="repeat_interval"]:checked'
    );
    ...
    document
      .querySelectorAll(".interval-specific-settings")
      .forEach((settings) => {
  ...
  handleMonthlyTypeChange() {
    const selectedType = document.querySelector(
      'input[name="monthly_type"]:checked'
    );
  ...
  updatePreview() {
    ...
    if (selectedInterval) {
      intervalElement.textContent = this.getIntervalDescription(
  ...
  calculateNextPublication() {
    ...
    const maxIterations = 1000; // Лимит
    ...
    if (iterations >= maxIterations) {
      console.error(
        "Слишком много итераций при вычислении следующей публикации"
      );
      return null;
    }
  ...
  saveSettings() {
    ...
    try {
      ...
      if (
        ...
        typeof this.onSettingsChanged === "function"
      ) {
    ...
    if (selectedInterval.value === "monthly") {
      ...
      if (monthlyType) {
        ...
        if (monthlyType.value === "date") {
          settings.monthlyDate = parseInt(
            document.getElementById("monthlyDate").value
          );
        } else if (monthlyType.value === "weekday") {
          settings.monthlyWeek = parseInt(
            document.getElementById("monthlyWeek").value
          );
          settings.monthlyWeekday = parseInt(
            document.getElementById("monthlyWeekday").value
          );
        }
      }
    }
  ...
  loadSettings(settings) {
    ...
    const intervalRadio = document.querySelector(
      `input[name="repeat_interval"][value="${settings.interval}"]`
    );
    if (intervalRadio) {
    ...
    if (settings.interval === "weekly" && settings.weekDay) {
      const dayRadio = document.querySelector(
        `input[name="weekly_day"][value="${settings.weekDay}"]`
      );
      if (dayRadio) {
    ...
    if (settings.interval === "monthly") {
      if (settings.monthlyType) {
        const typeRadio = document.querySelector(
          `input[name="monthly_type"][value="${settings.monthlyType}"]`
        );
        if (typeRadio) {
  ...
  resetModal() {
    ...
    const dailyRadio = document.querySelector(
      'input[name="repeat_interval"][value="daily"]'
    );
    if (dailyRadio) {
  ...
  hide() {
    if (this.modal) {
      ...
      if (repeatSettingsBtn) {
        setTimeout(() => repeatSettingsBtn.focus(), 300);
      }
