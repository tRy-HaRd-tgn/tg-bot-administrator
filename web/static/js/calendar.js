...
class CalendarManager {
    ...
    renderCalendar() {
        ...
        const startingDayOfWeek = firstDay.getUTCDay() === 0 ? 7 : firstDay.getUTCDay(); // Понедельник = 1
        ...
        for (let day = 1; day <= daysInMonth; day++) {
            ...
            dayEl.addEventListener('click', () => this.showDayEvents(year, month + 1, day));
    ...
    loadCalendarStats() {
        ...
        fetch('/api/calendar/events')
            ...
            .catch(error => {
                ...
                this.showLoading('calendar-content', false);
                this.showError('calendar-content', 'Не удалось загрузить события календаря');
            });
    }    // Загрузка сегодняшних кампаний
    loadTodayCampaigns() {
    ...
    renderTodayCampaigns() {
        ...
        if (todayEvents.length === 0) {
            todayContent.innerHTML = '<div class="no-campaigns-today">На сегодня нет запланированных кампаний</div>';
        } else {
            let html = '';
            ...
            todayEvents.forEach(event => {
                ...
                html += `
                    <div class="today-campaign-item" onclick="calendarManager.showEventDetails(${JSON.stringify(event).replace(/"/g, '&quot;')})">
                        <div class="today-campaign-time">
                            <i class="fas fa-clock"></i>
                            ${event.time || '00:00'}
                        </div>
                        <div class="today-campaign-details">
                            <div class="today-campaign-title">${event.title}</div>
                            <div class="today-campaign-info">
                                <div class="today-campaign-status">
                                    <i class="fas fa-circle" style="font-size: 8px; color: ${event.status === 'active' ? 'var(--apple-green)' : 'var(--apple-gray-1)'}"></i>
                                    <span class="badge ${statusClass}">${event.status || 'Активна'}</span>
                                </div>
                                ${event.chats ? `<div class="today-campaign-chats">
                        ...
                        <div class="today-campaign-actions">
                            <button class="today-campaign-button" onclick="window.location.href='/campaigns/${event.campaign_id}/edit'; event.stopPropagation();">
    ...
    renderEvents() {
        ...
        this.events.forEach(event => {
            try {
                ...
                if (year === this.currentDate.getUTCFullYear() && month === this.currentDate.getUTCMonth() + 1) {
    ...
    showDayEvents(year, month, day) {
        ...
        modalTitle.textContent = `События на ${day} ${this.getMonthName(month - 1)} ${year}`;
        
        if (dayEvents.length === 0) {
            modalBody.innerHTML = '<div class="no-events">На этот день нет запланированных событий</div>';
        } else {
            // Создаем более подробное отображение событий
            modalBody.innerHTML = dayEvents.map(event => `
                <div class="day-event-item ${this.getEventClass(event.type)}" onclick="calendarManager.showEventDetails(${JSON.stringify(event).replace(/"/g, '&quot;')})">
                    <div class="event-time">${this.formatTime(event.time || '00:00')} UTC</div>
                    <div class="event-details">
                        <div class="event-title">${event.title}</div>
                        <div class="event-type">Тип: ${this.getEventTypeName(event.type)}</div>
                        ...
                        <div class="event-campaign">
                            ...
                            <span class="event-campaign-name">${event.campaign_name || event.title}</span>
                        </div>
                        ${event.chats ? `<div class="event-chats">
        ...
        modalBody.innerHTML = `
            <div class="event-detail-section">
                <strong>Дата и время (UTC):</strong> ${this.formatDateTime(event.date)} ${event.time || '00:00'}
            </div>
            <div class="event-detail-section">
            ...
            ${buttonsInfo}
            <div class="event-detail-section">
                <strong>ID кампании:</strong> ${event.campaign_id || 'Не указан'}
            </div>
            <div class="event-detail-actions">
                <button class="btn btn-sm btn-outline-primary" onclick="window.location.href='/campaigns/${event.campaign_id}/edit'">
